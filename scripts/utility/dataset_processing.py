...
def get_foot_positions(joint_pos, model, data, foot_frames):
    ...
    for i, frame in enumerate(foot_frames):
        foot_positions[3*i:3*(i+1)] = data.oMf[model.getFrameId(frame)].translation
...
# 45 cols for observation information plus 5 extra for random initialisation
...
# foot positions
anymal_model = pin.buildModelFromUrdf(current_dir + "/../../resources/models/anymal_c/urdf/model.urdf")
anymal_data = anymal_model.createData()
frame_list = ["LF_FOOT", "RF_FOOT", "LH_FOOT", "RH_FOOT"]

# swap joint positions columns to conform to pinocchio model
joint_states = states[joint_pos_list]
col_list = list(joint_states)
col_list[3], col_list[4], col_list[5], col_list[6], col_list[7], col_list[8] \
    = col_list[6], col_list[7], col_list[8], col_list[3], col_list[4], col_list[5]
joint_states = joint_states.reindex(columns=col_list)
...
# export to csv
np.savetxt("../../resources/expert_data/expert_data_processed_w_feet.csv", obs, delimiter=",")
